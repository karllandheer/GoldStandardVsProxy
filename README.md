...
run ```python False_Positive_Rate_Multi_Alpha.py```. Note that False_Positive_Rate_Multi_Alpha and False_Negative_Rate_Multi_Alpha can take a very long time to run (several days). If you want to just test, set N = 5000 (or something like this). The numbers will change slightly, but it will instead run much quicker to test.
